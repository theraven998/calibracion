// RespiracionTable.tsx
import React, { useState, useEffect } from "react";

export interface RespiracionData {
...
interface Props {
  onDataChange: (data: RespiracionData) => void;
}

const RespiracionTable = ({ onDataChange }: Props) => {
  const [patrones] = useState<string[]>(["20", "30", "40", "50"]);
        ...
        <tbody>
          {patrones.map((patron, idx) => (
            <tr key={idx}>
              ...
              <td>
                <span className="calc-value">0</span>
              </td>
            </tr>
          ))}
        </tbody>
...
export default RespiracionTable;
